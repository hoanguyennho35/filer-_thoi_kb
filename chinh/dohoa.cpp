...
doc sv;
	int hocphan;cout<<"nhap so hoc phan ";cin>>hocphan;char *a5;
	ifstream a;
	ifstream b;
	int ik=0;ik=coutline();ik=(ik-2)/(hocphan+1);
	int ik1=0;
	char *d;
	b.open("D:/xep_tkb/chinh/data_dohoa/thu.txt",ios_base::in);
	string thu;
	int gd=DETECT,gm;
	initwindow(900,900);
...
//	cout<<"noi dung cua chuoi d "<<d;
	...
	c.open("D:/xep_tkb/chinh/data_dohoa/sosanh.txt",ios_base::in);string nhap1;int a1;getline(a,nhap); char k2;	//doc dau xuong hang
	getline(a,nhap);			//loc bo phan dau locthu
//	char *z1=(char*) ik;
//	outtextxy(750,750,z1);
	while(1){xoa();
	...
	free(d);i1++;}delay(2);a>>nhap;ik1++;
	if(ik1!=ik)
	{
	cout<<"nhan phim bat ky de tip ";k2=getche();}
	else	{cout<<"end";fflush(stdin);k2=getche();break;}
	}
closegraph();
c.close();
	b.close();
